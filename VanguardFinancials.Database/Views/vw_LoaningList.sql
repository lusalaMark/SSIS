IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='vw_LoaningList') BEGIN
   EXEC('CREATE view [dbo].[vw_LoaningList] as  select top 1 * from vfin_customers')
END
GO
/****** Object:  View [dbo].[vw_LoaningList]    Script Date: 9/14/2014 6:39:00 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dbo].[vw_LoaningList]
WITH SCHEMABINDING 
AS
SELECT        dbo.vfin_Branches.Description AS Branch, dbo.vfin_LoanProducts.Description AS LoanProduct, dbo.vfin_LoanPurposes.Description AS LoanPurpose, 
                         SPACE(10) AS FileReference, dbo.vfin_LoanCases.CaseNumber, dbo.vfin_LoanCases.AmountApplied, dbo.vfin_LoanCases.ReceivedDate, 
                         ...
                         CASE dbo.vfin_LoanProducts.LoanRegistration_LoanProductSection WHEN '47803' THEN 'BackOffice' WHEN '47802' THEN 'FrontOffice' ELSE 'UnKnown' END
                          AS LoanProductSection, REPLACE(STR(dbo.vfin_Customers.SerialNumber, 6), SPACE(1), '0') AS MembershipNumber, 
                         dbo.vfin_Customers.Individual_PayrollNumbers AS PersonalFileNumber, dbo.vfin_LoanCases.AppraisedBy, dbo.vfin_LoanCases.AppraisedDate, 
                         ...
                         dbo.vfin_LoanCases.DisbursedAmount, dbo.vfin_LoanCases.MonthlyPaybackAmount, dbo.vfin_LoanCases.TotalPaybackAmount, 
                         dbo.vfin_LoanCases.LoanProductInvestmentsBalance, dbo.vfin_LoanCases.LoanProductLoanBalance, dbo.vfin_LoanCases.TotalLoansBalance, 
                         CASE dbo.vfin_LoanCases.LoanInterest_CalculationMode WHEN 512 THEN 'Reducing Balance' WHEN 513 THEN 'Straight Line' WHEN 514 THEN 'Amortization'
                          ELSE 'Unknown' END AS LoanInterest_CalculationMode, 
