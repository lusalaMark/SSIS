IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='sp_DebitCardsExpiredReport') BEGIN
   EXEC('CREATE PROC [dbo].[sp_DebitCardsExpiredReport] as')
END
GO
/****** Object:  StoredProcedure [dbo].[sp_DebitCardsExpiredReport]    Script Date: 9/13/2014 4:11:05 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[sp_DebitCardsExpiredReport]
...
SELECT        RIGHT('00' + CAST(LTRIM(RTRIM(dbo.vfin_Branches.Code)) AS VARCHAR(5)), 5) + '-' + RIGHT('00000' + CAST(LTRIM(RTRIM(dbo.vfin_Customers.SerialNumber))
                         ...
                         CASE [Individual_Salutation] WHEN '48814' THEN 'Mr' WHEN '48815' THEN 'Mrs' WHEN '48816' THEN 'Miss' WHEN '48817' THEN 'Dr' WHEN '48818' THEN 'Prof' WHEN '48819'
                          ...
                          + ' ' + dbo.vfin_Customers.Individual_FirstName + ' ' + dbo.vfin_Customers.Individual_LastName AS FullName, dbo.vfin_DebitCards.Type, dbo.vfin_DebitCards.CardNumber, 
...
WHERE dbo.vfin_DebitCards.Expires>=@StartDate and dbo.vfin_DebitCards.Expires<=@Enddate and dbo.vfin_DebitCards.type=@type
END
